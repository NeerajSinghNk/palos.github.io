...
<link rel="stylesheet" type="text/css" href="css/style.css">
    ...
    <script src="java_script/contactBook.js"></script>
...
<%
	...
	if(user!=null)
	{
		...
		try
		{
			...
			int i=0;
			String query="select DISTINCT Book_type from books_oldnew where Phone='"+phoneCus+"'";
			...
			<div class="container">
			<%
			out.println("<form method='post' action='coustomer-contact.jsp'>");
			...
			%>
			 <label for="fname">First Name</label>
				<input type="text" id="fname" name="firstname" placeholder="Your name.." title="Name must be required" required>

				<label for="lname">Last Name</label>
				<input type="text" id="lname" name="lastname" placeholder="Your last name.." title="Provide last name" required>
						
			<%
			out.println("<label for='bt'>Book Type</label>");
			out.println("<select id='book' name='book' onchange='funAjax2(this.value)' title='Please select a book type' required> ");
		...
		out.println("<select id='bid' name='bid' onchange='titltData()' title='Please select book id for update request' required> "); 
				out.println("<option>select</option>");
		out.println("</select>");
		...
		<textarea id="subject" name="subject" placeholder="Write something what you want to change in your book.." style="height:200px" title="Write a comment what you want to update" required></textarea>
