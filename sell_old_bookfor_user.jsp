...
<%
		...
		try
	{
		...
		Connection con=DriverManager.getConnection("jdbc:odbc:commerce");
		Statement stmt=con.createStatement();
		ResultSet rs=stmt.executeQuery("select * from coustomer_register where Email='"+user+"'");
		int count=0;
		while(rs.next())
			{
				...
				out.println("<center><h3>Book selling form...</h3></center>");
						out.println("<div>");
							out.println("<form  action='upload_file.jsp' method='post' enctype='multipart/form-data' onsubmit='return SellUserBook();'>");
							
							
									out.println("<label><strong>Book Title:</strong></label>");
									...
									if(book_tit!=null) 
									{
										...
										//out.println("<input type='text' id='book-tit' value='"+book_tit+"' onload='return bookTitle();'>");
										session.removeAttribute("book_tit");
										
									}
							
							
									out.println("<label ><strong>Auther Name:</strong></label>");
									out.println("<input type='text' class='contact_input' id='auther_name' name='auther_name'  required>");
								
							
							
									out.println("<label ><strong>Book Publisher:</strong></label>");
									out.println("<input type='text' class='contact_input' id='book_pub' name='book_pub'  required>");
								
							
							
									out.println("<label ><strong>Date Published:</strong></label>");
									out.println("<input type='date' class='contact_input' id='date_pub' name='date_pub'  required>");
								
							
							
									out.println("<label ><strong>Price:</strong></label>");
									out.println("<input type='number' class='contact_input' id='price' name='price' required>");
								
							
							
									out.println("<label ><strong>Phone:</strong></label>");
									out.println("<input type='number' class='contact_input' id='phone' name='phone' value='"+ph+"'  required>");
								
							
									out.println("<label ><strong>Book Description:</strong></label>");
									out.println("<input type='text' class='contact_input' id='book_desc' name='book_desc'  required>");
								
							
							
									out.println("<label ><strong>Last Buy Date:</strong></label>");
									out.println("<input type='date' class='contact_input' id='lbd' name='lbd' >");
								
							
							
									out.println("<label ><strong>Book Photo:</strong></label>");
									out.println("<input type='file' class='contact_input' id='book_photo' name='book_photo' onchange='return validate_fileupload();' required>");
								
							
							
									out.println("<label ><strong>Book Type:</strong></label>");
		...
		try
			{
				...
				out.println("<div class='container'><table border='1' class='table table-hover' ><tr><th>Book id</th><th>Title</th><th>Auther</th><th>Description</th><th>Contact</th><th>Book Type</th><th>Price</th><th>Book</th></tr>");
